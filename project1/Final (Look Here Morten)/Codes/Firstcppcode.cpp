  ...
  string filename; //  This is a useful way to define a text string, alternatively you can use characters
    ...
    for (int i = 1; i <= exponent; i++){
      ...
      //   To be filled in during lab session
        double h = 1.0/((double) n); //make sure we don't have integrer division
        double hh = h*h; //h^2
        // Need to have felxible (dynamic memory allocation) memory usage
        double *d = new double[n+1]; //diagonal matrix elements
        //double *e = new double[n+1] //non-diagonal matrix elements
        double *f = new double[n+1]; //right hand side, a vectore f
        double *x = new double[n+1]; //x-values
        double *u = new double[n+1]; //solution u values
        ...
        finish = clock();
        double timeout = ((finish-start)/ ((double) CLOCKS_PER_SEC) );
